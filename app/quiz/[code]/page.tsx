...
export default function QuizPage() {
  ...
  const { user, updateUserStats } = useAuth() // Fix: Use updateUserStats from auth context
  const { getQuiz, submitQuizAnswer } = useQuiz() // Remove updateUserStats from quiz context
  ...
  const handleNextQuestion = async () => {
    ...
    if (currentQuestion < quiz.questions.length - 1) {
      ...
      try {
        ...
        await submitQuizAnswer(code as string, user.id, user.name, finalScore)
        ...
        // Fix: Use updateUserStats from auth context
        updateUserStats(
          {
            ...
            <CardHeader className="text-center">
              ...
              <div className="grid grid-cols-3 gap-4 text-center">
                <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                  <div className="text-2xl font-bold text-blue-500">{quiz.questions?.length || 0}</div>
